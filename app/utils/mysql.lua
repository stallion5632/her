...
local config = require("config.mysql_conf") or {}
...
function _M.connect(self, db)
    ...
    return db:connect({
        ...
        database = self.config.db_name or 'test',
...
function _M.new(self, opts)
    ...
    opts.host = opts.host or config.host
    opts.port = opts.port or config.port
    opts.user = opts.user or config.user
    opts.password = opts.password or config.password
    opts.db_name = opts.db_name or config.db_name
    opts.charset = (opts.charset or config.charset) or 'utf8'
    opts.timeout = opts.timeout or config.timeout
    opts.max_idle_timeout = opts.max_idle_timeout or config.max_idle_timeout
    opts.pool_size = opts.pool_size or config.pool_size
    self.config = opts
